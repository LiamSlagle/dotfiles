...
async function activate2(context) {
  ...
  function bootWorkspaceClient(folder) {
    ...
    const documentSelector = languageObj.map((language) => ({
      scheme: "file",
      language,
      pattern: `${import_coc3.Uri.parse(folder.uri).fsPath}/**/*`
    }));
    const clientOptions = {
      documentSelector,
  ...
  async function didOpenTextDocument(document) {
    ...
    folder = getOuterMostWorkspaceFolder(folder);
    try {
      const configFiles = await (0, import_fast_glob.default)(import_path.default.join(import_coc3.Uri.parse(folder.uri).fsPath, "**/" + CONFIG_GLOB), {
